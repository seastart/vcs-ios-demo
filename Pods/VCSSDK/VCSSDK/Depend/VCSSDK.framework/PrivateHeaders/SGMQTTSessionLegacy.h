...
@interface SGMQTTSession(Create)
 ...
 * and other vulnerabilities. you may need to set securityPolicy to properly value(see SGMQTTSSLSecurityPolicy.h for more detail).
 *
 ...
 * SGMQTTSSLSecurityPolicy.
 * And if you use self-signed server certificates, your need to set property: SGMQTTSSLSecurityPolicy.allowInvalidCertificates=YES
 * (see SGMQTTSSLSecurityPolicy.h for more detail).
 *
 ...
 * @param keepAliveInterval The Keep Alive is a time interval measured in seconds. The SGMQTTClient ensures that the interval between Control Packets being sent does not exceed the Keep Alive value. In the  absence of sending any other Control Packets, the Client sends a PINGREQ Packet.
 ...
 * @code
 ...
 NSString* certificate = [[NSBundle bundleForClass:[SGMQTTSession class]] pathForResource:@"certificate" ofType:@"cer"];
 SGMQTTSSLSecurityPolicy *securityPolicy = [SGMQTTSSLSecurityPolicy policyWithPinningMode:MQTTSSLPinningModeCertificate];
 ...
 SGMQTTSession *session = [[SGMQTTSession alloc]
 ...
 @endcode
 */
- (SGMQTTSession *)initWithClientId:(NSString *)clientId
                         userName:(NSString *)userName
                         password:(NSString *)password
                        keepAlive:(UInt16)keepAliveInterval
                   connectMessage:(SGMQTTMessage *)theConnectMessage
