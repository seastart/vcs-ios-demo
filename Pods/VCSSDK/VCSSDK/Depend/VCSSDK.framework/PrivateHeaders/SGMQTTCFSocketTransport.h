...
/** SGMQTTCFSocketTransport
 * implements an SGMQTTTransport on top of CFNetwork
 */
@interface SGMQTTCFSocketTransport : SGMQTTTransport <SGMQTTTransport, SGMQTTCFSocketDecoderDelegate, SGMQTTCFSocketEncoderDelegate>
...
/** reads the content of a PKCS12 file and converts it to an certificates array for initWith...
 ...
 @code
 NSString *path = [[NSBundle bundleForClass:[SGMQTTClientTests class]] pathForResource:@"filename"
 ofType:@"p12"];
 
 NSArray *myCerts = [SGMQTTCFSocketTransport clientCertsFromP12:path passphrase:@"passphrase"];
 if (myCerts) {
 
 self.session = [[SGMQTTSession alloc] init];
 ...
